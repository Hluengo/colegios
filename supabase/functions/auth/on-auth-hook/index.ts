...
async function handleNewUser(user: AuthUser) {
  ...
  if (provider && provider !== 'email') {
    console.log(`Rejected user ${user.id}: OAuth provider '${provider}' not allowed. Only email registration is permitted.`)
    throw new Error(`Registration with ${provider} is not allowed. Please use email and password.`)
  }
  ...
  if (!tenantSlug) {
    console.log(`No tenant_slug provided for user ${user.id}, skipping profile creation`)
    return
  }
  ...
  console.log(`Profile created for user ${user.id} with role ${role} in tenant ${tenant.id}`)
}
