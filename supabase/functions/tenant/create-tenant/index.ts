...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.slug || !body.name || !body.email) {
      return new Response(
        JSON.stringify({ error: 'slug, name, and email are required' }),
        {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        },
    ...
    if (version) {
      await supabase.from('tenant_versions').insert({
        tenant_id: tenant.id,
...
async function createDefaultCatalogs(tenantId: string) {
  ...
  const defaultCatalogs = [
    ...
    // Categorías
    {
      catalog_type: 'conduct_categories',
      key: 'leve',
      label: 'Leve',
      display_order: 1,
    },
