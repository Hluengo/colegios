...
export default function InformeCasoDocument({ caso, seguimientos = [] }) {
  ...
  const fechaCierre =
    caso?._supabaseData?.due_process_closed_at ||
    caso?.fields?.Fecha_Cierre ||
    caso?._supabaseData?.closed_at;

  const rutEstudiante =
    caso?.fields?.Estudiante_Responsable?.rut ||
    caso?._supabaseData?.students?.rut ||
    'N/A';

  const medidaFinal = caso?._supabaseData?.final_disciplinary_measure || null;
  const resolucionFinal = caso?._supabaseData?.final_resolution_text || null;
  const cierrePor =
    caso?._supabaseData?.closed_by_name ||
    caso?.fields?.Responsable_Registro ||
    null;
  const cierrePorRol =
    caso?._supabaseData?.closed_by_role || caso?.fields?.Rol_Responsable || null;
