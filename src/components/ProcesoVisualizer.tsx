export default function ProcesoVisualizer({
  ...
  return (
    <div className="w-full">
      ...
      <div className="md:hidden overflow-x-auto pt-6 pb-2 -mx-3 px-3 border-t border-dashed border-slate-200">
        <div className="flex flex-row gap-6 min-w-max">
          {stages.map((stageKey, index) => {
            ...
            return (
              <div
                key={stageKey}
                className={`flex flex-col items-center text-center transition-all ${isCurrent ? '-mt-1' : ''} min-w-[72px]`}
              >
                <div
                ...
                <div
                  className={`mt-3 text-xs font-semibold leading-tight line-clamp-3 max-w-[72px] ${isCurrent ? 'text-brand-700' : 'text-slate-600'}`}
                >
                  {stageName}
                </div>
