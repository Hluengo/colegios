...
function groupByStage(followups = []) {
  ...
  for (const [_k, arr] of map.entries()) {
    arr.sort((a, b) =>
      String(a?.fields?.Fecha || '').localeCompare(String(b?.fields?.Fecha || '')),
    );
  }
  return map;
}
        ...
        return (
          <div key={stageKey} className="border-b last:border-b-0 border-slate-100">
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                      count > 0 ? 'bg-white text-slate-600 border-slate-200 shadow-sm' : 'bg-slate-100 text-slate-400 border-transparent'
                    }`}
                  >
                    {count}
                  </span>
              ...
              <span className={`text-slate-400 text-lg transition-transform duration-200 ${isOpen ? 'rotate-180 text-brand-500' : ''}`}>
                ▾
              </span>
            </button>
