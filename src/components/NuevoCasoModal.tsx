...
export default function NuevoCasoModal({ onClose, onSaved }) {
  ...
  async function guardarCaso() {
    // Validación crítica: tenant_id es requerido
    if (!tenantId) {
      push({
        type: 'error',
        title: 'Error de configuración',
        message: 'No se encontró el tenant. Por favor, recarga la página.',
      });
      logger.error('⚠️ tenantId es null/undefined al guardar caso');
      return;
    }
    ...
    try {
      ...
      logger.debug('✅ estudianteId:', estudianteId);
      ...
      emitDataUpdated(); // Notificar a CasosActivos que se refresqe
      onSaved?.();
      onClose?.();
    } catch (e) {
      logger.error(e);
  ...
  return (
    ...
    >
      <div className="w-full max-w-5xl mx-auto relative space-y-4 max-h-[92dvh]">
        <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-0 relative flex flex-col h-full max-h-[92dvh] overflow-hidden">
          {guardando && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-brand-200 text-slate-700 cursor-pointer"
                  style={{
                    colorScheme: 'light'
                  }}
                />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {(categoriasByType[tipo] || []).map((texto) => (
                  <button
                    ...
                    aria-pressed={conductas.includes(texto)}
                  >
                    {texto}
                  </button>
                ))}
          ...
          <div className="px-4 sm:px-6 py-4 border-t border-slate-100 bg-gradient-to-r from-brand-50/70 to-transparent sticky bottom-0 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-brand-50 w-full sm:w-auto"
            >
              Cancelar
            </button>
            <button
              type="button"
              onClick={guardarCaso}
              disabled={guardando || loading || !!error || tenantLoading || !tenantId}
              className="px-5 py-2.5 rounded-xl bg-brand-600 text-white font-semibold hover:bg-brand-700 disabled:opacity-50 w-full sm:w-auto"
            >
              {guardando ? 'Guardando…' : tenantLoading ? 'Cargando...' : 'Guardar Caso'}
            </button>
          </div>
        </div>
      </div>
