...
export function AnimateOnMount({
  ...
  const animations: Record<string, CSSProperties> = {
    ...
    'slide-up': { opacity: isVisible ? 1 : 0, transform: `translateY(${isVisible ? 0 : 20}px)` },
    'slide-down': { opacity: isVisible ? 1 : 0, transform: `translateY(${isVisible ? 0 : -20}px)` },
    scale: { opacity: isVisible ? 1 : 0, transform: `scale(${isVisible ? 1 : 0.9})` },
  };
...
export function ProgressBar({
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      {showLabel && (
        <span className="text-xs text-surface-500 mt-1">{Math.round(percentage)}%</span>
      )}
    </div>
  );
}
...
export function Tabs({ tabs, defaultTab, className = '' }: TabsProps) {
  ...
  useEffect(() => {
    ...
    if (!tabs.some((t) => t.id === activeTab)) {
      setActiveTab(defaultTab && tabs.some((t) => t.id === defaultTab) ? defaultTab : tabs[0].id);
    }
