...
export function Divider({ label, orientation = 'horizontal', className = '' }: DividerProps) {
  ...
  return <div className={`h-px bg-surface-200 ${className}`} role="separator" />;
}
...
export function CardWrapper({
  ...
  return (
    <div
      className={`card ${paddings[padding]} ${onClick ? 'cursor-pointer hover:shadow-lg' : ''} ${className}`}
      onClick={onClick}
  ...
  return (
    <div
      className={`
        ...
        ${className}
      `}
      style={gap !== undefined ? { gap: typeof gap === 'number' ? `${gap * 0.25}rem` : gap } : undefined}
    >
      {children}
    </div>
  );
...
export function Spacer({ size = 4 }: SpacerProps) {
  return <div style={{ height: typeof size === 'number' ? `${size * 0.25}rem` : size }} />;
}
...
export function VStack({ children, gap = 4, className = '' }: VStackProps) {
  return (
    <div className={`flex flex-col ${className}`} style={{ gap: `${gap * 0.25}rem` }}>
      {children}
    </div>
  );
}
