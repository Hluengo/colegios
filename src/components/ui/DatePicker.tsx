...
export function DatePicker({
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {isOpen && !disabled && (
        <div
          className="absolute right-0 sm:left-0 sm:right-auto z-50 mt-2 p-3 sm:p-4 bg-white rounded-2xl border border-surface-200 shadow-surface-xl animate-scale-in"
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  disabled={disabled}
                  className={`
                    min-h-[36px] p-1.5 sm:p-2 text-sm rounded-lg transition-colors
...
export function DateRangePicker({
  ...
  const [tempStart] = useState<Date | null>(startDate);
  const [tempEnd] = useState<Date | null>(endDate);
  const containerRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {isOpen && (
        <div
          className="absolute right-0 sm:left-0 sm:right-auto z-50 mt-2 p-3 sm:p-4 bg-white rounded-2xl border border-surface-200 shadow-surface-xl"
          style={{ width: 'min(20rem, calc(100vw - 1rem))' }}
        >
