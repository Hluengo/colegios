...
export default function Sidebar({
  ...
  const shouldShowTenantSlug =
    Boolean(tenant?.slug?.trim()) && !hasConfiguredTenantName;
  ...
  useEffect(() => {
    const onPointerDown = (event: MouseEvent) => {
      ...
      if (
        ...
        target.closest('[data-user-menu-root="true"]')
      ) {
        return;
      }
      setUserMenuOpen(false);
  ...
  return (
    <>
      ...
      <aside
        className={`hidden lg:flex flex-col m-2.5 mr-0 relative z-20
          glass-panel
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 flex lg:hidden">
          ...
          />
          <aside className="relative w-[min(88vw,320px)] bg-white h-full shadow-2xl flex flex-col overflow-hidden">
            {/* Mobile Header */}
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              ...
              <button
                type="button"
                onClick={onClose}
                className="p-2 rounded-full hover:bg-slate-100 text-slate-500 tap-target"
              >
                <X size={20} />
              </button>
            </div>
