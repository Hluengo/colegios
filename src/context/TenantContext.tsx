  ...
  const fetchTenantData = async () => {
    try {
      ...
      if (profile.role === 'platform_admin') {
        ...
        if (fallbackTenant) {
          ...
          if (fallbackTenant.id !== profile.tenant_id) {
            ...
            if (updateProfileError) {
              logger.warn('No se pudo actualizar tenant_id fallback para platform_admin:', updateProfileError)
            }
          }
          return
        }
        ...
        setTenant(null)
        setError(new Error('No hay colegios activos para asignar al administrador de plataforma'))
        return
      }
