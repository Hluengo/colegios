...
// Helper functions
export async function getTenantCachedStats<T>(tenantId: string): Promise<T | null> {
  const key = `${CACHE_STRATEGIES.tenant_stats.prefix}${tenantId}`
  return cache.get<T>(key)
}

export async function setTenantCachedStats<T>(tenantId: string, data: T): Promise<void> {
  const key = `${CACHE_STRATEGIES.tenant_stats.prefix}${tenantId}`
...
export async function getCachedCatalogs<T>(tenantId: string, catalogType: string): Promise<T | null> {
  const key = `${CACHE_STRATEGIES.catalogs.prefix}${tenantId}:${catalogType}`
  return cache.get<T>(key)
}

export async function setCachedCatalogs<T>(tenantId: string, catalogType: string, data: T): Promise<void> {
  const key = `${CACHE_STRATEGIES.catalogs.prefix}${tenantId}:${catalogType}`
