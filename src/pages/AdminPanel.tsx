...
const SETTING_PRESETS: SettingPreset[] = [
  { key: 'ui.primary_color', label: 'Color primario', type: 'string', defaultValue: '#2563eb', description: 'Color principal de botones y acentos.' },
  { key: 'ui.secondary_color', label: 'Color secundario', type: 'string', defaultValue: '#1e40af', description: 'Color de apoyo en elementos secundarios.' },
  { key: 'notifications.case_created_email', label: 'Mail al crear caso', type: 'boolean', defaultValue: 'true', description: 'Envía correo al registrar un caso.' },
  { key: 'notifications.case_updated_email', label: 'Mail al actualizar caso', type: 'boolean', defaultValue: 'true', description: 'Envía correo cuando cambia un caso.' },
  { key: 'cases.default_responsible_role', label: 'Cargo responsable por defecto', type: 'string', defaultValue: 'Inspectoría', description: 'Cargo sugerido al registrar casos.' },
  { key: 'workflow.auto_close_days', label: 'Cierre automático (días)', type: 'number', defaultValue: '30', description: 'Días sin actividad para cerrar automáticamente.' },
  { key: 'branding.footer_message', label: 'Mensaje en pie de página', type: 'string', defaultValue: 'Convivencia Escolar 2026', description: 'Texto institucional en pie de página.' },
  { key: 'integrations.webhook_url', label: 'Webhook de integración', type: 'string', defaultValue: '', description: 'Endpoint externo para eventos del sistema.' },
  { key: 'integrations.webhook_enabled', label: 'Webhook habilitado', type: 'boolean', defaultValue: 'false', description: 'Activa/desactiva envío de eventos webhook.' },
  { key: 'reports.export_fields', label: 'Campos exportables', type: 'json', defaultValue: '["id","incident_date","student_name","status"]', description: 'Lista de campos en exportes.' },
];
...
export default function AdminPanel() {
  ...
  const [catalogTab, setCatalogTab] = useState<'types' | 'catalog' | 'actions' | 'sla'>('types');
  ...
  const [editingSetting, setEditingSetting] = useState<{ id: string; valueText: string } | null>(null);
  ...
  const [settingTypeFilter, setSettingTypeFilter] = useState<'all' | SettingInputType>('all');
  const [settingFormError, setSettingFormError] = useState('');
  const [newUser, setNewUser] = useState({ email: '', fullName: '', role: 'user', department: '' });
  ...
  const [editingAuditNotes, setEditingAuditNotes] = useState<Record<string, string>>({});

  const run = async (title: string, fn: () => Promise<void>, okMessage?: string) => {
    try {
      await fn();
      if (okMessage) push({ type: 'success', title, message: okMessage });
    } catch (err: any) {
      push({ type: 'error', title, message: err?.message || `Error en ${title.toLowerCase()}` });
    }
  };
  ...
  const buildSettingValue = (input: typeof newSetting) => {
    ...
    if (input.type === 'number') {
      ...
      if (!Number.isFinite(n)) throw new Error('El valor numérico no es válido');
      return n;
    }
    if (input.type === 'json') {
      try {
  ...
  const getSettingCategory = (key: string) => (String(key || '').split('.')[0] || 'custom').toLowerCase();
  ...
  const importCatalogCsv = async () => {
    if (!tenantId) return;
    if (!catalogImportPreview.length) throw new Error('No hay filas para importar');
    ...
    const missing = requiredHeaders[catalogTab].filter((header) => !catalogImportHeaders.includes(header));
    if (missing.length) {
      throw new Error(`CSV inválido. Faltan columnas requeridas: ${missing.join(', ')}`);
    }
  ...
  const load = async () => {
    ...
    try {
      ...
      setSettings(st || []);
      setCatalogStats({
        conductTypes: (ct || []).length,
        conductCatalog: (cc || []).length,
        actionTypes: (at || []).length,
        stageSla: (sla || []).length,
      });
      setConductTypes(ct || []);
  ...
  const courses = useMemo(() => [...new Set(sortedStudents.map((s) => s.course).filter(Boolean))], [sortedStudents]);
  ...
  const settingCategoryOptions = useMemo(
    () => ['all', ...Array.from(new Set(settings.map((s) => getSettingCategory(s.setting_key)).filter(Boolean)))],
    [settings],
  );
  const settingTypeOptions = useMemo(
    () => ['all', ...Array.from(new Set(settings.map((s) => detectSettingType(s.setting_value)).filter(Boolean)))],
    [settings],
  );
  ...
  const visibleSettings = useMemo(
    () =>
      filteredSettings.filter((s) => {
        const byCategory =
          settingCategoryFilter === 'all' || getSettingCategory(s.setting_key) === settingCategoryFilter;
        const byType = settingTypeFilter === 'all' || detectSettingType(s.setting_value) === settingTypeFilter;
        return byCategory && byType;
      }),
    [filteredSettings, settingCategoryFilter, settingTypeFilter],
  );
  const auditActionOptions = useMemo(
    () => ['all', ...Array.from(new Set(audit.map((r) => String(r.action || '').toUpperCase()).filter(Boolean)))],
    [audit],
  );
  const auditTableOptions = useMemo(
    () => ['all', ...Array.from(new Set(audit.map((r) => String(r.table_name || '').toLowerCase()).filter(Boolean)))],
    [audit],
  );
  const filteredAudit = useMemo(() => {
    const q = auditSearch.trim().toLowerCase();
    return audit.filter((r) => {
      const byAction = auditActionFilter === 'all' || String(r.action || '').toUpperCase() === auditActionFilter;
      const byTable = auditTableFilter === 'all' || String(r.table_name || '').toLowerCase() === auditTableFilter;
      const bySearch =
        !q ||
  ...
  const basicCount = students.filter((s) => /b[áa]sica|basico|básico|1°|2°|3°|4°|5°|6°|7°|8°/i.test(s.level || s.course || '')).length;
  ...
  return (
    <div className="min-h-screen" style={{ background: 'radial-gradient(1200px 500px at 100% -120px, rgba(75, 96, 124, 0.08), transparent 65%), linear-gradient(180deg, #f5f7fb 0%, #fbfcfe 100%)' }}>
      <div className="mx-auto flex max-w-[1500px] gap-4 px-4 py-4 sm:px-6">
        <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} hidden shrink-0 glass-panel p-3 lg:block`}>
          <button onClick={() => setSidebarOpen((v) => !v)} className="mb-3 rounded-lg border border-slate-200 p-2 text-slate-700 hover:bg-slate-100">
            <Menu size={16} />
          </button>
          <nav className="space-y-1">
            {visibleTabs.map((t) => {
              ...
              return (
                <button key={t.id} onClick={() => setTab(t.id)} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm ${active ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100'}`}>
                  <Icon size={16} />
                  {sidebarOpen && t.label}
                </button>
              );
        ...
        <main className="min-w-0 flex-1 space-y-4">
          <header className="glass-panel p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-wide text-slate-500">Panel / {visibleTabs.find((t) => t.id === tab)?.label || 'Administración'}</p>
          ...
          <section className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            <StatCard title="Total Alumnos" value={students.length} icon={<GraduationCap size={16} />} />
            <StatCard title="Alumnos en Básica" value={basicCount} icon={<BookOpen size={16} />} tone="blue" />
            <StatCard title="Alumnos en Media" value={mediaCount} icon={<LibraryBig size={16} />} tone="violet" />
            <StatCard title="Última actualización" value={loading ? 'Actualizando...' : new Date().toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })} icon={<Clock3 size={16} />} tone="emerald" />
          </section>
                          ...
                          onChange={(e) => {
                            ...
                            reader.onload = (event) => {
                              ...
                              const parsed = lines.map(line => {
                                const [first_name, last_name, rut, course, level] = line.split(',').map(s => s?.trim().replace(/^"|"$/g, ''));
                                return { first_name, last_name, rut, course, level };
                        ...
                        return (
                          <tr key={s.id} className="border-t border-slate-200 hover:bg-slate-50">
                            ...
                            <td className="px-4 py-3 text-right">
                              ...
                              {!isEditing ? (
                                <button className="ml-2 rounded-lg border border-slate-200 px-3 py-1 text-xs" onClick={() => setEditing((m) => ({ ...m, [s.id]: { ...s } }))}>Editar</button>
                              ) : (
                                <>
                                  <button className="rounded-lg bg-slate-900 px-3 py-1 text-xs text-white" onClick={() => run('Estudiantes', async () => { await updateStudent(s.id, row); setEditing((m) => { const n = { ...m }; delete n[s.id]; return n; }); await load(); }, 'Estudiante actualizado')}>Guardar</button>
                                  <button className="ml-2 rounded-lg border border-slate-200 px-3 py-1 text-xs" onClick={() => setEditing((m) => { const n = { ...m }; delete n[s.id]; return n; })}>Cancelar</button>
                                </>
                              )}
                            </td>
                          </tr>
          ...
          {tab === 'branding' && (
            <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">
                Datos por completar:{' '}
                <span className={`font-semibold ${pendingBrandingFields > 0 ? 'text-amber-700' : 'text-emerald-700'}`}>
                  {pendingBrandingFields}
                </span>
              </div>
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                <input className={brandingFieldClass(branding.name)} placeholder="Nombre" value={branding.name} onChange={(e) => setBranding((s) => ({ ...s, name: e.target.value }))} />
                <input className={brandingFieldClass(branding.email)} placeholder="Correo" value={branding.email} onChange={(e) => setBranding((s) => ({ ...s, email: e.target.value }))} />
                <input className={brandingFieldClass(branding.phone)} placeholder="Telefono" value={branding.phone} onChange={(e) => setBranding((s) => ({ ...s, phone: e.target.value }))} />
                <input className={brandingFieldClass(branding.address)} placeholder="Direccion" value={branding.address} onChange={(e) => setBranding((s) => ({ ...s, address: e.target.value }))} />
              </div>
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                <label className="text-sm">Subir logo<input type="file" accept="image/*" className="mt-1 block" onChange={(e) => run('Storage', async () => { const f = e.target.files?.[0]; if (!f || !tenantId) return; const { url } = await uploadTenantBrandAsset(tenantId, f, 'logo'); setBranding((s) => ({ ...s, logo_url: url })); }, 'Logo cargado')} /></label>
                <label className="text-sm">Subir favicon<input type="file" accept="image/*" className="mt-1 block" onChange={(e) => run('Storage', async () => { const f = e.target.files?.[0]; if (!f || !tenantId) return; const { url } = await uploadTenantBrandAsset(tenantId, f, 'favicon'); setBranding((s) => ({ ...s, favicon_url: url })); }, 'Favicon cargado')} /></label>
              </div>
              <div className="flex flex-wrap gap-2">
                <button className="btn-primary px-4 py-2" onClick={() => run('Colegio', async () => { if (!tenantId) return; await updateTenantBranding(tenantId, branding); await refetch(); }, 'Colegio actualizado')}>Guardar Colegio</button>
                <button className="rounded-lg bg-slate-100 px-4 py-2" onClick={() => run('Catalogos', async () => { if (!tenantId) return; await applyCollegeCatalogs(tenantId); await load(); }, 'Catálogos aplicados')}>Aplicar Catalogo RPC</button>
              </div>
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">
                <p className="font-semibold">¿Qué hace "Aplicar Catalogo RPC"?</p>
                <p className="mt-1">
                  Ejecuta la función de publicación de catálogos del colegio en Supabase y deja activos los datos que se usarán en formularios y flujos del sistema
                  (tipos de conducta, catálogo de conductas, SLA de etapas y tipos de acción).
                </p>
                <p className="mt-2 font-semibold">Requisitos recomendados antes de ejecutarlo</p>
                <ul className="mt-1 list-disc pl-5 space-y-1">
                  <li>Tener definido el colegio activo correcto (si eres platform admin).</li>
                  <li>Completar y guardar la marca básica del colegio (nombre, correo, teléfono, dirección).</li>
                  <li>Haber ejecutado el alta inicial del colegio cuando es un tenant nuevo.</li>
                </ul>
              </div>
              {isPlatformAdmin && (
          ...
          {tab === 'platform' && isPlatformAdmin && (
            <div className="space-y-4">
              <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-4">
                <h3 className="font-semibold text-indigo-900">Configuración de plataforma del colegio</h3>
                <p className="mt-1 text-sm text-indigo-800">
                  Estos parámetros impactan suscripción, localización y habilitación de módulos del colegio activo.
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-4">
                ...
                <div>
                  <p className="text-sm font-medium text-slate-800">Funciones habilitadas</p>
                  <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {Object.entries(platformConfig.features).map(([key, enabled]) => (
                      <label key={key} className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 text-sm">
                        <span className="text-slate-700">{PLATFORM_FEATURE_LABELS[key] || key.replace(/_/g, ' ')}</span>
                        <input
                          type="checkbox"
                      ...
                      run(
                        ...
                        async () => {
                          ...
                          await updateTenantBranding(tenantId, {
                            subscription_status: platformConfig.subscription_status,
                            subscription_plan: platformConfig.subscription_plan,
                            trial_end_date: platformConfig.trial_end_date || null,
                ...
                {!showUserForm ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="font-semibold text-slate-900">Invitar nuevo usuario</h3>
                      <button onClick={() => setShowUserForm(false)} className="text-slate-500 hover:text-slate-700">
                        <X size={18} />
                      </button>
                    </div>
                      ...
                      {users.map((u) => {
                        ...
                        return (
                          <tr key={u.id} className="border-t border-slate-200 hover:bg-slate-50">
                            ...
                            <td className="px-4 py-3">
                              <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${u.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                                {u.is_active ? 'Activo' : 'Inactivo'}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-slate-500">
                              {u.last_login_at ? new Date(u.last_login_at).toLocaleDateString('es-CL') : 'Nunca'}
                            </td>
                            <td className="px-4 py-3 text-right">
                              {!isEditing ? (
                                <button
                                  className="rounded-lg border border-slate-200 px-3 py-1 text-xs"
                                  onClick={() => setEditing((m) => ({ ...m, [u.id]: { ...u } }))}
                                >
                                  Editar
                                </button>
                              ) : (
                                <>
                                  <button
                                    ...
                                    onClick={() => run('Usuarios', async () => {
                                      ...
                                      setEditing((m) => { const n = { ...m }; delete n[u.id]; return n; });
                                      await load();
                                    }, 'Usuario actualizado')}
                                  >
                                    Guardar
                                  ...
                                  <button
                                    className="ml-2 rounded-lg border border-slate-200 px-3 py-1 text-xs"
                                    onClick={() => setEditing((m) => { const n = { ...m }; delete n[u.id]; return n; })}
                                  >
                                    Cancelar
                                  </button>
                                </>
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-3">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-xs text-slate-600">
                    Columnas esperadas: {expectedCsvHeaders[catalogTab].join(', ')}
                  </span>
                  {catalogImportFileName && (
                    <span className="text-xs text-slate-500">Archivo: {catalogImportFileName}</span>
                  )}
                </div>
                {catalogImportPreview.length > 0 && (
                  <div className="mt-3 space-y-2">
                    <div className="flex flex-wrap items-center gap-2 text-sm text-slate-700">
                      <span>{catalogImportPreview.length} filas listas para importar</span>
                      <button
                      ...
                      >
                        {catalogImporting ? 'Importando...' : 'Importar a tabla'}
                      </button>
                      <button
                        ...
                        <tbody>
                          {catalogImportPreview.slice(0, 10).map((row, idx) => (
                            <tr key={idx} className="border-t">
                              {catalogImportHeaders.map((header) => (
                                <td key={`${idx}-${header}`} className="px-2 py-1">{String(row[header] || '')}</td>
                              ))}
                            </tr>
                          ))}
                          {catalogImportPreview.length > 10 && (
                            <tr className="border-t">
                              <td colSpan={catalogImportHeaders.length || 1} className="px-2 py-1 text-center text-slate-500">
                                ...y {catalogImportPreview.length - 10} filas más
                              </td>
                            </tr>
                          )}
                        </tbody>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                {catalogTab === 'types' && (
                  <CatalogEditor
                    ...
                    onAdd={async (item) => {
                      if (!tenantId) return;
                      await run('Catálogos', async () => {
                        await upsertConductType(tenantId, item);
                        await load();
                      }, 'Tipo de conducta guardado');
                    }}
                    onDelete={async (id) => {
                      if (!window.confirm('¿Eliminar este tipo de conducta?')) return;
                      await run('Catálogos', async () => {
                    ...
                    fields={[
                      { key: 'key', label: 'Clave', placeholder: 'Ej: agresion_fisica, respeto' },
                      { key: 'label', label: 'Etiqueta', placeholder: 'Ej: Agresión Física, Respeto' },
                      { key: 'color', label: 'Color', placeholder: 'Ej: #ef4444' },
                ...
                {catalogTab === 'catalog' && (
                  <CatalogEditor
                    ...
                    fields={[
                      { key: 'conduct_type', label: 'Tipo', placeholder: 'Seleccionar tipo de conducta' },
                      { key: 'conduct_category', label: 'Conducta', placeholder: 'Ej: Agresión verbal, Respeto, etc.' },
                      { key: 'sort_order', label: 'Orden', type: 'number' },
                    ]}
                ...
                {catalogTab === 'actions' && (
                  <CatalogEditor
                    ...
                    fields={[
                      { key: 'key', label: 'Clave', placeholder: 'Ej: suspension, advertencia, citacion' },
                      { key: 'label', label: 'Etiqueta', placeholder: 'Ej: Suspensión, Amonestación, Citación' },
                      { key: 'description', label: 'Descripción', placeholder: 'Descripción de la acción' },
                      { key: 'sort_order', label: 'Orden', type: 'number' },
                    ]}
                  />
                )}
                {catalogTab === 'sla' && (
                  <div className="space-y-3">
                    <p className="text-sm text-slate-600">
                      Configura los tiempos de respuesta para cada etapa del proceso.
                      El valor numérico siempre representa <strong>días corridos</strong>.
                    </p>
                    <div className="flex items-center justify-between">
                      <h3 className="font-semibold text-slate-900">SLA Etapas ({stageSla.length})</h3>
                      {!showSlaForm && (
                        <button
                    ...
                    {showSlaForm && (
                      <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                        <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
                          ...
                          <button
                            ...
                            onClick={() =>
                              run('SLA', async () => {
                                if (!tenantId) return;
                                if (!newSla.stage_key.trim()) throw new Error('Debes ingresar stage_key');
                                await upsertStageSla(tenantId, {
                                  stage_key: newSla.stage_key.trim(),
                                  days_to_due: Number(newSla.days_to_due || 0),
                                });
                        ...
                        <p className="mt-2 text-xs text-slate-500">
                          Ejemplo: si ingresas <strong>5</strong>, el plazo de la etapa será de <strong>5 días</strong>.
                        </p>
                        <button
                          ...
                          {stageSla.map((sla) => (
                            <SlaRow
                              ...
                              onSave={async (updated) => {
                                if (!tenantId) return;
                                await run('SLA', async () => {
                                  await updateStageSla(tenantId, sla.stage_key, updated);
                                  await load();
                                }, 'SLA actualizado');
                              }}
                              onDelete={async () => {
                                if (!tenantId) return;
                                if (!window.confirm(`¿Eliminar SLA de etapa "${sla.stage_key}"?`)) return;
                                await run('SLA', async () => {
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-4">
                ...
                <p className="mt-1 text-sm text-blue-800">
                  Define claves (<code>setting_key</code>) y valores (<code>setting_value</code>) para controlar marca, reglas y comportamiento.
                </p>
                <p className="mt-2 text-xs text-blue-700">
                  Flujo recomendado: <strong>1)</strong> elegir plantilla, <strong>2)</strong> ajustar clave y tipo, <strong>3)</strong> guardar.
                </p>
              </div>

              <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-4">
                <div className="flex items-center justify-between gap-2">
                  <h4 className="font-medium text-slate-900">Plantillas rápidas</h4>
                  <span className="text-xs text-slate-500">Selecciona una para autocompletar</span>
                </div>
                    ...
                    <button
                      ...
                      onClick={() =>
                        setNewSetting({
                          key: preset.key,
                          type: preset.type,
                          valueText: preset.type === 'boolean' ? '' : preset.defaultValue,
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!slug) return;
                      setNewSetting((s) => ({ ...s, key: `${settingModule}.${slug}` }));
                    }}
                  >
                    Generar clave
                  </button>
                ...
                <p className="text-xs text-slate-500">
                  Convención recomendada: <code>módulo.nombre_parametro</code> (ej: <code>workflow.auto_close_days</code>).
                </p>
                <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
                  <input
                ...
                {newSetting.type === 'boolean' ? (
                  <label className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700">
                    <input
                      ...
                      onChange={(e) => setNewSetting((s) => ({ ...s, valueBool: e.target.checked }))}
                    />
                    Valor habilitado
                  </label>
                ) : (
                      ...
                      run(
                        ...
                        async () => {
                          ...
                          if (!/^[a-z0-9._-]+$/i.test(newSetting.key.trim())) {
                            setSettingFormError('La clave solo admite letras, números, punto, guion y guion bajo');
                            return;
                          }
                          const value = buildSettingValue(newSetting);
                          setSettingFormError('');
                          await upsertTenantSetting(tenantId, newSetting.key.trim(), value);
                          setNewSetting({ key: '', valueText: '', type: 'string', valueBool: true });
                          setSettingName('');
                          await load();
                        },
                        'Parámetro guardado',
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="mb-3 grid grid-cols-2 gap-2 md:grid-cols-4">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                    ...
                    <p className="text-lg font-semibold text-slate-900">{settings.length}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-wide text-slate-500">Booleanos</p>
                    <p className="text-lg font-semibold text-slate-900">{settings.filter((s) => detectSettingType(s.setting_value) === 'boolean').length}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-wide text-slate-500">JSON</p>
                    <p className="text-lg font-semibold text-slate-900">{settings.filter((s) => detectSettingType(s.setting_value) === 'json').length}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-wide text-slate-500">Categorías</p>
                    <p className="text-lg font-semibold text-slate-900">{settingCategoryOptions.filter((x) => x !== 'all').length}</p>
                  </div>
                </div>
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  {settingsByCategory.slice(0, 8).map((row) => (
                    <button
                      ...
                      onClick={() => setSettingCategoryFilter((prev) => (prev === row.category ? 'all' : row.category))}
                    >
                ...
                {visibleSettings.length > 0 ? (
                  <div className="space-y-2">
                    {visibleSettings.map((s) => (
                      <div key={s.id} className="flex items-start gap-3 border-t pt-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="font-mono font-medium text-slate-800">{s.setting_key}</span>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  run(
                                    ...
                                    async () => {
                                      if (!tenantId) return;
                                      await upsertTenantSetting(tenantId, s.setting_key, parseValue(editingSetting.valueText));
                        ...
                        <button className="text-red-600 hover:text-red-800 shrink-0" onClick={() => run('Parámetros', async () => { await deleteTenantSetting(s.id); await load(); }, 'Parámetro eliminado')}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    ))}
                  ...
                  <p className="text-slate-500 text-center py-4">No hay parámetros configurados</p>
                )}
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl bg-purple-50 border border-purple-200 p-4">
                ...
                <p className="text-sm text-purple-800 mt-1">
                  Este módulo muestra un historial de todas las acciones realizadas en el sistema. 
                  Permite rastrear cambios, creaciones y eliminaciones para cumplimiento y seguridad.
                </p>
                <div className="mt-2 text-xs text-purple-700">
                  <strong>Información registrada:</strong> Usuario responsable, acción realizada, tabla afectada, 
                  identificador del registro, fecha y hora del cambio.
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                    ...
                    onClick={() =>
                      run(
                        ...
                        async () => {
                          ...
                          setNewAuditEntry({ action: 'MANUAL', tableName: 'manual', note: '' });
                          await load();
                        },
                ...
                <div className="mb-3 flex flex-wrap items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3">
                  <span className="text-sm text-amber-900">Purgar auditoría anterior a</span>
                  <input
                    type="number"
                    min={1}
                    ...
                    onClick={() =>
                      run(
                        ...
                        async () => {
                          ...
                          before.setDate(before.getDate() - Number(auditPurgeDays || 90));
                    ...
                    <tbody>
                      {filteredAudit.map((r) => (
                        <tr key={r.id} className="border-t hover:bg-slate-50">
                          ...
                          <td className="px-4 py-2 font-mono text-xs text-slate-500">{r.record_id ? r.record_id.slice(0, 8) + '...' : '—'}</td>
                          <td className="px-4 py-2">
                            <div className="flex items-center gap-2">
                              <input
                                className="h-8 w-52 rounded border border-slate-200 px-2 text-xs"
                                value={editingAuditNotes[r.id] ?? r.admin_note ?? ''}
                                onChange={(e) => setEditingAuditNotes((m) => ({ ...m, [r.id]: e.target.value }))}
                              />
                              <button
                                ...
                                onClick={() =>
                                  run(
                                    'Auditoría',
                                    async () => {
                                      await updateAuditLogNote(r.id, editingAuditNotes[r.id] ?? r.admin_note ?? '');
                                      await load();
                                    },
                                    'Nota actualizada',
                                  )
                            ...
                            {r.old_values || r.new_values ? (
                              <details className="group">
                                ...
                                <div className="mt-1 text-xs bg-slate-100 p-2 rounded font-mono max-w-xs">
                                  {r.old_values && <div className="text-red-600">Antes: {JSON.stringify(r.old_values)}</div>}
                                  {r.new_values && <div className="text-green-600">Después: {JSON.stringify(r.new_values)}</div>}
                                </div>
                              </details>
                            ) : '—'}
                          </td>
                      ...
                      {filteredAudit.length === 0 && (
                        <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-500">No hay registros de auditoría</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
...
function StatCard({ title, value, icon, tone = 'slate' }: { title: string; value: string | number; icon: React.ReactNode; tone?: 'slate' | 'blue' | 'violet' | 'emerald' }) {
  const toneClass = {
...
function LevelBadge({ level }: { level: string }) {
  ...
  const isMedia = /media|i medio|ii medio|iii medio|iv medio|1 medio|2 medio|3 medio|4 medio/i.test(text);
  const isBasica = /b[áa]sica|basico|básico|1°|2°|3°|4°|5°|6°|7°|8°/i.test(text);
  if (isMedia) return <span className="inline-flex rounded-full border border-violet-200 bg-violet-50 px-2 py-0.5 text-xs font-medium text-violet-700">Media</span>;
  if (isBasica) return <span className="inline-flex rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">Básica</span>;
  return <span className="inline-flex rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">{text}</span>;
}

function RoleBadge({ role }: { role: string }) {
  const roleConfig: Record<string, { bg: string; text: string; label: string }> = {
    platform_admin: { bg: 'bg-purple-100', text: 'text-purple-700', label: 'Administrador de plataforma' },
    tenant_admin: { bg: 'bg-amber-100', text: 'text-amber-700', label: 'Administrador' },
    user: { bg: 'bg-slate-100', text: 'text-slate-600', label: 'Usuario' },
  };
  const config = roleConfig[role] || roleConfig.user;
  return (
    <span className={`inline-flex rounded-full border border-slate-200 px-2 py-0.5 text-xs font-medium ${config.bg} ${config.text}`}>
      {config.label}
    </span>
  );
}

type FieldConfig = { key: string; label: string; placeholder?: string; type?: 'text' | 'number' };
...
function CatalogEditor({
  ...
  const handleAdd = async () => {
    ...
    await onAdd({ ...newItem, sort_order: Number(newItem.sort_order || 0), [activeField]: true });
    setNewItem({});
    setShowForm(false);
  };
      ...
      {showForm && (
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="mt-2 flex gap-2">
            <button onClick={handleAdd} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-sm text-white">Guardar</button>
            <button onClick={() => { setShowForm(false); setNewItem({}); }} className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm">Cancelar</button>
          </div>
        </div>
      )}
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-t">
                {fields.map((f) => (
                  <td key={f.key} className="px-3 py-2">{item[f.key]}</td>
                ))}
                <td className="px-3 py-2">
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${item[activeField] ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
            ...
            {items.length === 0 && (
              <tr><td colSpan={fields.length + 2} className="px-3 py-4 text-center text-slate-500">No hay elementos</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <td className="px-4 py-2 text-right">
        <div className="flex items-center justify-end gap-2">
          ...
          <button
            ...
            disabled={saving || deleting}
            className="inline-flex items-center gap-1 rounded-lg border border-red-200 px-3 py-1 text-xs text-red-700 hover:bg-red-50 disabled:opacity-50"
          >
            <Trash2 size={12} />
...
function ActionBadge({ action }: { action: string }) {
  ...
  const config = actionConfig[action] || { bg: 'bg-slate-100', text: 'text-slate-600', label: action };
  return (
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <div className="overflow-auto rounded-lg border border-slate-200">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-t">{r.map((c, j) => <td key={`${i}-${j}`} className="px-3 py-2">{c}</td>)}</tr>
            ))}
          </tbody>
        </table>
      </div>
