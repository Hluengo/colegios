...
function Stepper({ activeKey }) {
  return (
    <div className="overflow-x-auto">
      <div className="flex items-center justify-start sm:justify-center gap-4 sm:gap-6 min-w-max pb-1">
        {CIERRE_STEPS.map((s, idx) => {
          ...
          return (
            <div key={s.key} className="flex items-center gap-2">
              ...
              <div
                className={`text-sm font-semibold whitespace-nowrap ${active ? 'text-brand-700' : 'text-slate-600'}`}
              >
                {s.label}
              </div>
            </div>
  ...
  return (
    <div className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 gap-3">
      <div className="flex items-center gap-3 min-w-0">
        <CheckCircle2
          className={`w-5 h-5 shrink-0 ${ok ? 'text-emerald-600' : 'text-slate-400'}`}
        />
...
export default function CierreCasoPage() {
  ...
  async function finalizarYCerrar() {
    ...
    const accepted = await confirm({
      ...
      message:
        '¿Seguro que deseas cerrar este caso? Esta acción quedará registrada.',
      variant: 'warning',
    });
    if (!accepted) return;
    ...
    <div className="h-full w-full">
      ...
      <div className="max-w-5xl mx-auto">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-4 sm:px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-brand-50/70 to-transparent flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[11px] text-slate-600 font-semibold truncate flex items-center gap-1">
                <button
                  type="button"
                  className="inline-flex items-center hover:underline tap-target"
                  onClick={() => navigate('/')}
                >
                  Inicio
                </button>
            ...
            <button
              onClick={() => navigate(-1)}
              className="p-2 rounded-lg hover:bg-slate-100 text-slate-500 tap-target shrink-0"
              aria-label="Cerrar"
              title="Volver"
            >
              <X className="w-5 h-5" />
          ...
          <div className="px-4 sm:px-6 py-5">
            ...
            <div className="mt-4 flex items-center justify-end gap-3">
              {prevStep && (
                <button
                  ...
                  className="inline-flex items-center px-2 py-1 text-xs font-semibold text-slate-600 hover:underline tap-target"
                >
                  Volver
                </button>
              )}
              {nextStep && (
                <button
                  ...
                  className="inline-flex items-center px-2 py-1 text-xs font-semibold text-brand-700 hover:underline tap-target"
                >
                  Siguiente
                </button>
              )}
            ...
            <div ref={documentacionRef} className="mt-8">
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-gradient-to-r from-brand-50/70 to-transparent p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="min-w-0">
                ...
                <button
                  onClick={generarPDF}
                  className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl border border-slate-200 bg-white text-slate-800 text-sm font-semibold hover:bg-brand-50 w-full sm:w-auto"
                >
            ...
            <div className="mt-8 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3">
              <button
                onClick={() => navigate(-1)}
                className="px-4 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-brand-50 w-full sm:w-auto"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={isClosed}
                className={`px-5 py-2 rounded-xl text-sm font-semibold text-white w-full sm:w-auto ${
                  isClosed
