...
export async function startSeguimiento(caseId) {
  try {
    ...
    // Obtener el tenant_id del caso
    const { data: caseData, error: caseError } = await withRetry(() =>
      supabase
        .from('cases')
        .select('tenant_id')
        .eq('id', caseId)
    ...
    const tenantId = caseData.tenant_id;
  ...
  try {
    ...
    const { data, error } = await withRetry(() =>
      supabase
        ...
        .insert([row])
        .select(
          `id, case_id, action_date, action_type, process_stage, detail, responsible, observations, due_date, created_at, description`,
        )
        .single(),
    );
    if (error) throw error;
...
export async function getAllControlAlertas(tenantId: string | null = null) {
  ...
  const { data, error } = await withRetry(() =>
    query.order('dias_restantes', { ascending: true }),
  );
  if (error) throw error;
...
export async function getConductTypes({
  ...
  // conduct_types no tiene tenant_id en esquema actual
  if (tenantId) {
    void tenantId;
  }
...
export async function getConductCatalog({
  ...
  // conduct_catalog no tiene tenant_id en esquema actual
  if (tenantId) {
    void tenantId;
  }
  if (activeOnly) q = q.eq('active', true);
...
export async function getInvolucrados(caseId) {
  // Tabla: public.involucrados (case_id -> cases.id)
  if (!caseId) return [];
