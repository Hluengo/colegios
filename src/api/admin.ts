...
export async function createStudent(tenantId: string, payload: Record<string, any>) {
  const { data, error } = await supabase
    .from('students')
    .insert({
      tenant_id: tenantId,
...
export async function updateStudent(studentId: string, payload: Record<string, any>) {
  const { data, error } = await supabase
    .from('students')
    .update(payload)
    .eq('id', studentId)
...
export async function importStudents(tenantId: string, students: Array<{
  first_name?: string;
  last_name?: string;
  rut?: string;
  course?: string;
...
export async function listConductTypes(tenantId: string) {
  const { data, error } = await supabase
    .from('conduct_types')
    .select('*')
...
export async function upsertConductType(tenantId: string, payload: Record<string, any>) {
  const row = {
    key: payload.key,
    label: payload.label,
...
export async function listConductCatalog(tenantId: string) {
  const { data, error } = await supabase
...
export async function deleteConductCatalogRow(id: string) {
  const { error } = await supabase.from('conduct_catalog').delete().eq('id', id);
  if (error) throw error;
}
...
export async function upsertActionType(tenantId: string, payload: Record<string, any>) {
  const row = {
    tenant_id: tenantId,
    key: payload.key,
    label: payload.label,
...
export async function upsertStageSla(_tenantId: string, payload: Record<string, any>) {
  const row = {
...
export async function inviteTenantUser(tenantId: string, payload: {
  email: string;
  fullName?: string;
  role?: string;
  department?: string;
...
export async function listAuditLogs(tenantId: string, limit = 100) {
  const { data, error } = await supabase
    .from('audit_logs')
    .select('id, created_at, action, table_name, record_id, user_id, old_values, new_values, admin_note')
    .eq('tenant_id', tenantId)
    .order('created_at', { ascending: false })
    .limit(limit);
  if (error) throw error;
